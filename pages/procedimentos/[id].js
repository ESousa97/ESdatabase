...
const ProcedureDetails = () => {
  ...
  useEffect(() => {
    const fetchProcedure = async () => {
      ...
      try {
        const response = await fetch(`https://server-json-eight.vercel.app/api/procedure`);
  ...
  const preprocessContent = (content) => {
    ...
    let processedContent = content.replace(/\/Assets\/(IMC\d+\/IMC\d+__[1-9].png)/g, 'https://esdatabase.vercel.app/$1');
